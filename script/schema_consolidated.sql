...
-- ##########################################################################
-- [정본 통합 추가분] ddl.sql 이후 델타 + 제약조건
-- 엔티티 최종 스키마 기준. 빈 스키마(fresh)에서 위->아래로 무에러 실행 목표.
-- ##########################################################################

-- ===== 1. orders <-> order_item 관계 정정 (order_item.order_seq -> orders.seq) =====
ALTER TABLE orders DROP COLUMN order_item_seq CASCADE CONSTRAINTS;
ALTER TABLE order_item ADD order_seq NUMBER;
ALTER TABLE order_item MODIFY order_seq NUMBER NOT NULL;
ALTER TABLE order_item ADD CONSTRAINT fk_order_item_order
    FOREIGN KEY (order_seq) REFERENCES orders (seq);

-- ===== 2. product_wish: options_seq -> product_seq, status number -> varchar2 =====
ALTER TABLE product_wish DROP COLUMN options_seq CASCADE CONSTRAINTS;
ALTER TABLE product_wish ADD product_seq NUMBER;
ALTER TABLE product_wish MODIFY product_seq NUMBER NOT NULL;
ALTER TABLE product_wish ADD CONSTRAINT fk_product_wish_product
    FOREIGN KEY (product_seq) REFERENCES product (seq);
ALTER TABLE product_wish MODIFY status VARCHAR2(20) DEFAULT 'NORMAL';
ALTER TABLE product_wish MODIFY status NOT NULL;

-- ===== 3. seller.role (스프링 시큐리티) =====
ALTER TABLE seller ADD role VARCHAR2(50) DEFAULT 'ROLE_SELLER' NOT NULL;
...
CHECK (participation_seq IS NULL OR (coupon_discount = 0 AND hotdeal_discount = 0));
...
CREATE UNIQUE INDEX UQ_participation_active ON participation (
...
-- ===== 상품/옵션/리뷰/이미지/찜 CHECK — 이세빈 =====
ALTER TABLE product ADD CONSTRAINT chk_product_sale_status CHECK (sale_status IN ('ON_SALE','SOLD_OUT','STOPPED'));
ALTER TABLE product ADD CONSTRAINT chk_product_approval_status CHECK (approval_status IN ('PENDING','APPROVED','REJECTED'));
ALTER TABLE product ADD CONSTRAINT chk_product_hide_yn CHECK (hide_yn IN ('Y','N'));
ALTER TABLE product ADD CONSTRAINT chk_product_status CHECK (status IN ('NORMAL','DELETED'));
ALTER TABLE product ADD CONSTRAINT chk_product_price CHECK (price >= 0);
ALTER TABLE product ADD CONSTRAINT chk_product_view_count CHECK (view_count >= 0);
ALTER TABLE product ADD CONSTRAINT chk_product_avg_rating CHECK (avg_rating BETWEEN 0 AND 5);
ALTER TABLE product ADD CONSTRAINT chk_product_review_count CHECK (review_count >= 0);
ALTER TABLE product ADD CONSTRAINT chk_product_sales_count CHECK (sales_count >= 0);

ALTER TABLE options ADD CONSTRAINT chk_options_stock CHECK (stock >= 0);
ALTER TABLE options ADD CONSTRAINT chk_options_safety_stock CHECK (safety_stock >= 0);
ALTER TABLE options ADD CONSTRAINT chk_options_additional_price CHECK (additional_price >= 0);

ALTER TABLE product_request ADD CONSTRAINT chk_preq_type CHECK (request_type IN ('REGISTER','UPDATE'));
ALTER TABLE product_request ADD CONSTRAINT chk_preq_status CHECK (request_status IN ('PENDING','APPROVED','REJECTED'));
ALTER TABLE product_request ADD CONSTRAINT chk_preq_process_date CHECK (process_date IS NULL OR process_date >= request_date);

ALTER TABLE review ADD CONSTRAINT chk_review_rating CHECK (rating IN (1,2,3,4,5));
ALTER TABLE review ADD CONSTRAINT chk_review_status CHECK (status IN ('NORMAL','DELETED','HIDDEN'));
ALTER TABLE review ADD CONSTRAINT chk_review_updated_date CHECK (updated_date IS NULL OR updated_date >= created_date);

ALTER TABLE review_image ADD CONSTRAINT chk_rimg_order CHECK (image_order > 0);
ALTER TABLE review_image ADD CONSTRAINT chk_rimg_file_type CHECK (file_type IN ('JPG','JPEG','PNG','WEBP'));
ALTER TABLE review_image ADD CONSTRAINT chk_rimg_status CHECK (status IN ('NORMAL','DELETED'));

ALTER TABLE product_image ADD CONSTRAINT chk_pimg_thumbnail_yn CHECK (thumbnail_yn IN ('Y','N'));
ALTER TABLE product_image ADD CONSTRAINT chk_pimg_order CHECK (image_order > 0);
ALTER TABLE product_image ADD CONSTRAINT chk_pimg_status CHECK (status IN ('NORMAL','DELETED'));

ALTER TABLE product_price_analysis ADD CONSTRAINT chk_ppa_avg CHECK (avg_price >= 0);
ALTER TABLE product_price_analysis ADD CONSTRAINT chk_ppa_min CHECK (min_price >= 0);
ALTER TABLE product_price_analysis ADD CONSTRAINT chk_ppa_max CHECK (max_price >= 0);
ALTER TABLE product_price_analysis ADD CONSTRAINT chk_ppa_min_max CHECK (min_price IS NULL OR max_price IS NULL OR max_price >= min_price);

ALTER TABLE product_wish ADD CONSTRAINT chk_pwish_member CHECK (member_seq > 0);
ALTER TABLE product_wish ADD CONSTRAINT CHK_PRODUCT_WISH_STATUS CHECK (status IN ('NORMAL','DELETED'));
ALTER TABLE product_wish ADD CONSTRAINT uk_product_wish_member_product UNIQUE (member_seq, product_seq);

ALTER TABLE seller ADD CONSTRAINT chk_seller_role CHECK (role = 'ROLE_SELLER');

-- ===== 배송/배송기록/허브/택배사/반품/카테고리 — 김민도 =====
ALTER TABLE delivery ADD CONSTRAINT ck_delivery_target CHECK ((orders_seq IS NOT NULL AND purchase_order_seq IS NULL) OR (orders_seq IS NULL AND purchase_order_seq IS NOT NULL));
ALTER TABLE delivery ADD CONSTRAINT ck_delivery_status CHECK (status IN ('READY','SHIPPING','DELIVERED','FAILED','DELAYED','CANCELED'));
ALTER TABLE delivery ADD CONSTRAINT ck_delivery_fee CHECK (distance_surcharge >= 0 AND total_delivery_fee >= 0);
ALTER TABLE delivery_history ADD CONSTRAINT ck_dh_lat CHECK (curr_latitude BETWEEN -90 AND 90);
ALTER TABLE delivery_history ADD CONSTRAINT ck_dh_lng CHECK (curr_longitude BETWEEN -180 AND 180);
ALTER TABLE hub ADD CONSTRAINT ck_hub_lat CHECK (latitude BETWEEN -90 AND 90);
ALTER TABLE hub ADD CONSTRAINT ck_hub_lng CHECK (longitude BETWEEN -180 AND 180);
ALTER TABLE delivery_company ADD CONSTRAINT ck_dc_fee CHECK (base_delivery_fee >= 0 AND monthly_fee >= 0);
ALTER TABLE return_request ADD CONSTRAINT ck_rr_status CHECK (status IN (0,1,2,3,4));
ALTER TABLE return_request ADD CONSTRAINT ck_rr_qty CHECK (return_quantity > 0);
ALTER TABLE returns ADD CONSTRAINT ck_returns_status CHECK (status IN ('READY','RETURNING','RETURNED'));
ALTER TABLE category ADD CONSTRAINT ck_category_depth CHECK (depth_level IN (0,1,2));
ALTER TABLE category ADD CONSTRAINT uq_category_name UNIQUE (parent_seq, category_name);

-- ===== 발주/재고이력/판매처 — 이찬희 =====
ALTER TABLE purchase_order ADD CONSTRAINT chk_po_status CHECK (status IN ('발주요청','입고완료','물품불량','입고지연','지연입고'));
ALTER TABLE purchase_order ADD CONSTRAINT chk_po_quantity CHECK (quantity >= 1);
ALTER TABLE purchase_order ADD CONSTRAINT chk_po_type CHECK (type IN ('일반','공동구매'));
ALTER TABLE stock_history ADD CONSTRAINT chk_sh_type CHECK (type IN ('IN','OUT'));
ALTER TABLE stock_history ADD CONSTRAINT chk_sh_quantity CHECK (quantity >= 1);
ALTER TABLE stock_history ADD CONSTRAINT chk_sh_before CHECK (before_stock >= 0);
ALTER TABLE stock_history ADD CONSTRAINT chk_sh_after CHECK (after_stock >= 0);
ALTER TABLE seller ADD CONSTRAINT chk_seller_supply_rate CHECK (supply_rate BETWEEN 1 AND 100);
